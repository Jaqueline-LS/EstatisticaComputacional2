...
esperado<-replicate(length(observado),1000/length(observado))

x.sq<-sum(((observado-esperado)^2)/esperado)

# Se os valores são iguais, a estatistica T~Qui-qua(10-1), 9
1-pchisq(x.sq, df=length(observado)-1)
...
# Outra forma de contar os pontos observados em cada categoria
...
diferencas.1<-diff(amostra4, lag = 1)
I.p<-(diferencas.1>0) 
I.p<-as.numeric(I.p)
I.n<-1-I.p
