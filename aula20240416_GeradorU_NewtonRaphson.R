...
{
  valor<-p-1/2
  sign(valor)*(abs(valor)/4)**(1/3) + 1/2 #raiz cubica de neg
}
qUser(0.7)

# Testes para a atividade 2
# qUser2<-function(p)
# { 
#   x<-1E200
...
curve(dUser, from=0, to=1, col="aquamarine3", lty=2, add=T, lwd=3)
...
# Newton-Raphson
# Plotar a função para ter uma ideia de onde é a raiz, chute inicial!
f1<-function(x) (x**2)-10
curve(f1, from=0, to=3.5)
abline(h=0, v=0, col="grey")
f1.linha<-function(x) 2*x # Derivada
